...
int main() {
    ...
    size_t n_centroids = 1500;
    size_t max_nodes_per_centroids = 2000;

    
    float* xb = fvecs_read("/home/clive/see/data/dataset/trip_distilbert/passages.fvecs", &d, &nb);
    ...
    index = faiss::read_index("/home/clive/see/data/dataset/trip_distilbert/tiptoe_centroids.index", 0);

    // faiss::write_index(index, "/home/clive/see/data/dataset/trip_distilbert/tiptoe_centroids.index");
    ...
    for(int qid = 0; qid < nq; qid++){
        ...
        // std::cout << "cluster: " << std::hex <<  clusters[cid] << std::endl;
        clusters[cid]->search(
            1, 
            xq + qid*d, 
            n_result, 
    ...
    ivecs_write("/home/clive/see/data/dataset/trip_distilbert/tiptoe_trip.ivecs", result, k, nq);
